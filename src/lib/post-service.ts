...
async function readPostsFromFile(): Promise<Post[]> {
  await ensurePostsFileExists(initialPostsData); // Pass initialPostsData here
  try {
    const jsonData = await fs.readFile(postsFilePath, 'utf-8');
    return JSON.parse(jsonData) as Post[];
  } catch (error) {
    console.error('Error reading posts file:', error);
    // If reading fails, return a copy of initialPostsData to try and recover/reset
    return JSON.parse(JSON.stringify(initialPostsData)); 
  }
}

async function writePostsToFile(posts: Post[]): Promise<void> {
  await ensureDir(dataDir); // Ensure directory exists before writing
  try {
...
const initialPostsData: Post[] = [
  ...
  },
  {
    ...
    "tags": ["celebrity", "brad pitt", "relationships"],
  ...
  },
  {
    ...
    "tags": ["viral", "tiktok", "gen z", "work culture"],
  ...
  },
  {
    ...
    "tags": ["newborn", "twins", "viral video", "family"],
  ...
  },
  {
    ...
    "tags": ["reality tv", "mama june", "weight loss", "transformation"],
  ...
  },
  {
    ...
    "tags": ["robin williams", "koko", "gorilla", "animals", "comedy"],
  ...
  },
  {
    ...
    "tags": ["singing", "talent show", "kids", "viral"],
