  ...
  try {
    ...
    if (count === 0 || (countError && countError.code === '42P01')) {
      ...
      const postsToInsert = postsFromFile.map(p => ({
        id: p.id, // ID from JSON is expected to be a string (UUID)
        slug: p.slug,
        title: p.title,
        date: p.date,
        content: p.content,
...
// Helper function to map Supabase row to Post, ensuring id is string
const mapDbRowToPost = (row: any): Post => {
...
export const getPostBySlug = async (slug: string): Promise<Post | undefined> => {
  ...
  return data ? mapDbRowToPost(data) : undefined;
};
...
export const getPostById = async (id: string): Promise<Post | undefined> => {
  ...
  const { data, error } = await supabase
    .from('posts')
    .select('*')
    .eq('id', id) // Supabase client handles string ID for UUID/numeric comparison correctly
    .single();
...
export const addPost = async (newPostData: Omit<Post, 'id' | 'date' | 'viewCount'> & { viewCount?: number }): Promise<Post> => {
  ...
  const postToInsert = {
    // id is typically auto-generated by Supabase for new rows (e.g., if it's a UUID with default)
...
export const updatePost = async (postId: string, updatedPostData: Partial<Omit<Post, 'id' | 'date'>>): Promise<Post | undefined> => {
  ...
  if (Object.keys(postToUpdate).length === 0) {
    ...
    return getPostById(postId); // Return current post data if no updates
  }

  const { data, error } = await adminSupabase
    .from('posts')
    .update(postToUpdate)
    .eq('id', postId) // postId is already a string here
    .select()
    .single();
...
export const incrementViewCount = async (postId: string): Promise<number | null> => {
  ...
  // The post_id_arg should match the type in your Supabase function. 
  // If your 'id' column in 'posts' table is UUID, then post_id_arg should be UUID.
  // If your 'id' column is INT, then post_id_arg should be INT.
  // Since our Post type and JSON use string UUIDs, we assume post_id_arg is UUID.
  // If the error indicates numeric IDs, the Supabase 'id' column itself might be numeric.
  // For now, we pass postId (string) as is; Supabase client handles UUID strings.
  // If 'id' column is numeric, RPC expects integer. `parseInt(postId)` would be needed if `postId` was stringified number.
  // However, the problem seems to be that `postId` is *already* a number when it shouldn't be.
  // The fix here ensures `postId` coming from `Post` objects is always string.
  ...
  if (error) {
    console.warn(
        ...
        `  3. The 'posts.id' column type might not match what 'increment_post_view_count' function's 'post_id_arg' expects (e.g., function expects int, but id is uuid).\n` +
        `  4. There could be network issues or temporary Supabase service problems.\n` +
        `  5. Incorrect RLS policies if SECURITY DEFINER is not used on the SQL function, though service_role should bypass RLS.\n` +
